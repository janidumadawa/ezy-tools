...
export default function TikTokPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 relative overflow-hidden">

      {/* Background Watermarks - hidden on mobile */}
 
      <div className="hidden md:block absolute top-0 left-[10px] pointer-events-none -z-10">
        <img
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 hover:border-gray-400 focus-within:border-black focus-within:shadow-lg transition-all">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 p-2 sm:p-3">
            <div className="flex items-center gap-2 sm:gap-3 flex-1">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-cyan-200 rounded-lg flex items-center justify-center flex-shrink-0">
                <img src="/logos/tik-tok.png" alt="TikTok Logo" className="w-4 h-4 sm:w-5 sm:h-5 object-contain" />
              </div>
              <input
                type="text"
                value={url}
            ...
            <button
              onClick={fetchVideo}
              disabled={loading}
              className="px-4 sm:px-6 py-2.5 sm:py-3 bg-black hover:bg-gray-800 disabled:bg-gray-300 text-white rounded-lg text-xs sm:text-sm font-semibold transition-all flex items-center justify-center gap-2 whitespace-nowrap"
            >
              {loading ? <Loader2 className="w-3 h-3 sm:w-4 sm:h-4 animate-spin" /> : <Download className="w-3 h-3 sm:w-4 sm:h-4" />}
              Analyze
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
          ...
          <div className="p-4 sm:p-6">
            ...
            <button
              ...
              className="w-full py-3 sm:py-3.5 bg-black hover:bg-gray-800 disabled:bg-gray-300 text-white rounded-lg text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2"
            >
              {downloading ? (
                ...
                <><Download className="w-3 h-3 sm:w-4 sm:h-4" /> Download Video</>
              )}
            </button>
