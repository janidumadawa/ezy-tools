  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500">Convert PPTX presentations to PDF</p>
      </div>

      {!file ? (
        <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-300 rounded-xl p-10 text-center cursor-pointer hover:border-orange-400">
          <Upload className="w-10 h-10 text-gray-400 mx-auto mb-3" />
          <p className="text-gray-600 font-medium">Click to upload PowerPoint file</p>
          <p className="text-sm text-gray-400">.pptx files supported</p>
          <input ref={fileInputRef} type="file" accept=".pptx" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" />
        </div>
      ) : !result ? (
        <div className="space-y-4">
          ...
          <button onClick={handleConvert} disabled={loading} className="w-full py-3 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-300 text-white rounded-lg font-semibold flex items-center justify-center gap-2">
      ...
      {result && (
        <div className="mt-6 p-6 bg-green-50 border border-green-200 rounded-xl text-center">
          ...
          <a href={`${API_URL}/file/${result.filename}`} download className="inline-flex items-center gap-2 mt-4 px-6 py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700">
