...
export default function CsvToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/csv-to-pdf`, formData)
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-2">CSV to PDF</h1>
        <p className="text-gray-500">Convert CSV spreadsheet files to PDF format</p>
      </div>

      {!file ? (
        <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-300 rounded-xl p-10 text-center cursor-pointer hover:border-teal-400">
          <Upload className="w-10 h-10 text-gray-400 mx-auto mb-3" />
          <p className="text-gray-600 font-medium">Click to upload CSV file</p>
          <p className="text-sm text-gray-400">.csv files supported</p>
        ...
        <div className="space-y-4">
          ...
          <button onClick={handleConvert} disabled={loading} className="w-full py-3 bg-teal-600 hover:bg-teal-700 disabled:bg-gray-300 text-white rounded-lg font-semibold flex items-center justify-center gap-2">
